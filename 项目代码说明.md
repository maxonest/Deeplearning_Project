...
|---|---|---:|---:|---|
| 后端服务 | `backend/` | 1,033 | 22.6% | API、启动状态、RAG 编排、模型推理、会话记忆 |
| 向量知识库 | `embeddings/` | 666 | 14.6% | 文本切块、Transformer 编码、FAISS 建库与查询 |
| 模型训练与独立推理 | `models/` | 619 | 13.6% | LoRA/QLoRA 训练、本地模型命令行测试 |
| 公共工具 | `utils/` | 384 | 8.4% | 配置、数据加载、提示词、语料清洗、日志 |
| React 前端源码 | `frontend/src/` | 1,189 | 26.1% | 页面、SSE 消费、Markdown、来源展示、响应式样式 |
| 自动化测试 | `tests/` | 671 | 14.7% | 后端、RAG、模型、Embedding、数据和启动脚本测试 |
| **合计** | 核心源码与测试 | **4,562** | **100%** | 不含启动脚本和配置清单 |
...
| 启动脚本合计 |  | **223** |
| 环境与项目配置 | `.env.example`、依赖文件、`pyproject.toml`、前端清单等 | 112 |

核心源码、测试和启动脚本合计约 **4,785 行**。
...
| 文件 | 行数 | 说明 |
|---|---:|---|
| `backend/app.py` | 342 | FastAPI 应用、启动生命周期、健康检查、普通与流式聊天接口 |
| `backend/llm.py` | 266 | 本地模型和 LoRA 加载、普通生成、流式生成 |
| `backend/rag.py` | 267 | 检索器、普通对话识别、RAG 提示词和回答编排 |
| `backend/memory.py` | 93 | 线程安全的多轮会话存储和历史压缩 |
| `backend/schemas.py` | 65 | Pydantic 请求和响应结构 |
| **合计** | **1,033** |  |
...
| `utils/config.py` | 113 | `.env` 配置读取、字段校验和路径解析 |
...
| 文件 | 行数 | 说明 |
|---|---:|---|
| `frontend/src/pages/index.jsx` | 392 | 主页面、健康轮询、SSE 解析、对话状态和交互 |
...
| 文件 | 行数 | 说明 |
|---|---:|---|
| `tests/test_app.py` | 219 | API、启动状态、知识库降级、LoRA 开关和模型预加载 |
...
| `tests/test_rag.py` | 84 | RAG 提示词、上下文截断、LoRA 开关和检索失败回退 |
...
| 文件 | 行数 | 用途 |
|---|---:|---|
| `.env.example` | 21 | 运行参数模板，复制为 `.env` 后生效 |
| `utils/config.py` | 113 | 将环境变量解析成类型安全的 `Settings` 对象 |
...
RAG 主体位于 `backend/rag.py`，共 267 行。
...
`backend/app.py` 是系统后端入口，共 342 行。
...
前端源码共 1,189 行，其中 610 行为样式，579 行为 React/JavaScript 逻辑。
...
项目共有 32 个测试函数、671 行测试代码。测试不依赖真实 9B 权重，也不要求已有 FAISS 索引。
...
从代码规模看，项目的主要复杂度集中在前端交互、向量知识库、训练循环和后端服务四个部分。其中前端源码约占 26.1%，后端约占 22.6%，向量知识库与模型训练合计约占 28.2%。测试代码占 14.7%，说明项目已对关键业务流程建立了基础自动化验证。
